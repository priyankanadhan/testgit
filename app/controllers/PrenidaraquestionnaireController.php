...
class PrenidaraquestionnaireController extends \Phalcon\Mvc\Controller {
	...
	public function getbyid() {
		...
		$category = QuestionsCategory::findFirstBycategory_name ( $input_data->category_name );
		 ...
		 ->leftjoin ( 'Options', 'Questions.id=Options.questions_id' )
	...
	public function create() {
		...
		$validation->add ( 'session_id', new PresenceOf ( [ 
				'message' => 'Session id is required' 
		] ) );
		$validation->add ( 'is_correct', new PresenceOf ( [ 
	...
	public function update() {
		...
		$messages = $validation->validate ( $question_option );
		if (count ( $messages )) :
			...
			foreach ( $question_option as $key => $value ) {
				$answers = Answers::findFirstBy ( array (
						"questions_id" => $value->questions_id,
						"nidara_kid_profile_id" => $input_data->nidara_kid_profile_id
				) );
				$answers->options_id = $value->options_id;
				$answers->modified_at = date ( 'Y-m-d H:i:s' );
				if (! $answers->save ()) {
