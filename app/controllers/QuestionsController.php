...
/**
	...
	public function getbycategoryname() {
		...
		if (empty ( $category_name )) :
			...
			$questionscategory = QuestionsCategory::findFirst ( 'category_name="' . $category_name . '"' )->toArray ();
			$category_id=$questionscategory['id'];	
		    $questions = Questions::find ( "questions_category_id=$category_id" );
			if ($questions) :
			...
			if ($question_category) {
				...
				foreach ( $question_category as $key => $value ) {
					...
					$answers = Answers::find ( array (
							"nidara_kid_profile_id" => $id,
							"questions_id" => $value->id 
					) );
					if (! empty ( $answers )) {
						$questions ['answers'] = array (
								"answer_id" => $answers->id,
								"option_id" => $answers->options_id 
						);
					}
					$result [] = $questions;
				}
